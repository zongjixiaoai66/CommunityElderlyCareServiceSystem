<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="服务种类" prop="fuwuzhonglei">
          <el-select :disabled="ro.fuwuzhonglei" v-model="ruleForm.fuwuzhonglei" placeholder="请选择服务种类">
            <el-option
                v-for="(item,index) in fuwuzhongleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="服务种类" prop="fuwuzhonglei">
	      <el-input v-model="ruleForm.fuwuzhonglei"
                placeholder="服务种类" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='fuwuzhonglei'){
            this.ruleForm.fuwuzhonglei = obj[o];
	    this.ro.fuwuzhonglei = true;
            continue;
          }
          if(o=='gongzuoshijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/fuwuzhonglei/fuwuzhonglei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.fuwuzhongleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.fuwuxiangqing = this.ruleForm.fuwuxiangqing.replace(reg,'../../../springboot654g2/upload');
        } else {
          this.$message.error(data.msg);
        }
      });

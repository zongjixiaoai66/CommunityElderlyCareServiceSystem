<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="物品种类" prop="wupinzhonglei">
          <el-select :disabled="ro.wupinzhonglei" v-model="ruleForm.wupinzhonglei" placeholder="请选择物品种类">
            <el-option
                v-for="(item,index) in wupinzhongleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/wupinzhonglei/wupinzhonglei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.wupinzhongleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.wupinxiangqing = this.ruleForm.wupinxiangqing.replace(reg,'../../../springboot654g2/upload');
        } else {
          this.$message.error(data.msg);
        }
      });

...
export default {
  ...
  methods: {
	...
	this.ruleForm.zixunneirong = this.ruleForm.zixunneirong.replace(reg,'../../../springboot654g2/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zixunzhongxin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zixunzhongxinCrossAddOrUpdateFlag = false;
						 this.parent.search();
